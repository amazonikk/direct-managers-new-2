...
function testBackend() {
  ...
  report.managers.forEach(function(manager) {
    manager.accounts.forEach(function(account) {
      account.records.forEach(function(record) {
        totalChats += Number(record.chats || 0);
        totalNumbers += Number(record.numbers || 0);
        totalComments += Number(record.comments || 0);
        if (!latestActivityDate || record.date > latestActivityDate) {
          latestActivityDate = record.date;
        }
      });
    });
  });
...
function parseAccountSheet_(manager, sheet, timezone) {
  ...
  const commentsProcessedColumn = findColumn_(columnHeaders, function(header) {
    return (
  ...
  for (let rowIndex = 0; rowIndex < values.length; rowIndex += 1) {
    ...
    // Порожні шаблонні дати часто містять формули, що повертають 0.
    // Такі рядки НЕ є реальною активністю і не повинні зсувати "останню дату" вперед.
    const hasData =
      chats > 0 ||
      (totalNumbersValue !== null && totalNumbersValue > 0) ||
      (commentsProcessedValue !== null && commentsProcessedValue > 0);
