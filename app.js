(() => {
  ...
  const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwOBe2zcSr-Gv84HxrQzHrQUBvRDU9H9m1Z_NwDIV0LdiJYoVMx2Ge5OxdrIwJ-w_omGw/exec";
  ...
  function renderNotice() {
    ...
    showNotice(
      `<strong>${escapeHtml(periodLabel(range))}.</strong> Вибрано ${active.length} ${plural(active.length, "акаунт", "акаунти", "акаунтів")}. Остання дата в даних: ${dateFmt.format(parseIsoDate(bounds.max))}.`,
      false,
      true
    );
  }
  ...
  function getDateBounds(entries = null) {
    ...
    const dates = source.flatMap(({ account }) => account.records.map((record) => record.date));
  ...
  function getChartPeriods() {
    ...
    const count = state.granularity === "day" ? 14 : state.granularity === "week" ? 10 : 12;
    const periods = [];
    ...
    for (let offset = count - 1; offset >= 0; offset -= 1) {
      if (state.granularity === "day") {
        ...
        const start = addDays(current.start, -offset * 7);
        const end = addDays(start, 6);
        periods.push({
          start,
          end,
        ...
        const month = addMonths(current.start.slice(0, 7), -offset);
        const range = monthRange(month);
        periods.push({
          ...range,
          key: month,
